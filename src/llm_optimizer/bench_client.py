...
def get_dataset(
    ...
    if dataset_name == "sharegpt":
        ...
        input_requests = sample_sharegpt_requests(
            dataset_path=args_dict.get("dataset_path", ""),
            num_requests=args_dict.get("num_prompts", 1000),
            tokenizer=tokenizer,
            fixed_output_len=args_dict.get("sharegpt_output_len"),
            context_len=args_dict.get("sharegpt_context_len"),
            prompt_suffix=args_dict.get("prompt_suffix", ""),
            apply_chat_template=args_dict.get("apply_chat_template", False),
        )
    elif dataset_name.startswith("random"):
        input_requests = sample_random_requests(
            input_len=args_dict.get("random_input_len", 1024),
            output_len=args_dict.get("random_output_len", 1024),
            num_prompts=args_dict.get("num_prompts", 1000),
            range_ratio=args_dict.get("random_range_ratio", 0.0),
            tokenizer=tokenizer,
            dataset_path=args_dict.get("dataset_path", ""),
            random_sample=dataset_name == "random",
            return_text=not tokenize_prompt,
        )
    elif dataset_name == "generated-shared-prefix":
        assert not tokenize_prompt
        input_requests = sample_generated_shared_prefix_requests(
            num_groups=args_dict.get("gsp_num_groups", 64),
            prompts_per_group=args_dict.get("gsp_prompts_per_group", 16),
        ...
        input_requests = sample_mmmu_requests(
            num_requests=args_dict.get("num_prompts", 1000),
            tokenizer=tokenizer,
            fixed_output_len=args_dict.get("random_output_len", 1024),
            random_sample=True,
        )
    else:
...
def gen_prompt(tokenizer, token_num):
    ...
    all_available_tokens = get_tokenizer_vocab(tokenizer)
...
def get_gen_prefix_cache_path(
    ...
    cache_key = (
        f"gen_shared_prefix_{args_dict.get('gsp_num_groups', 64)}_{args_dict.get('gsp_prompts_per_group', 16)}_"
        f"{args_dict.get('gsp_system_prompt_len', 2048)}_{args_dict.get('gsp_question_len', 128)}_{args_dict.get('gsp_output_len', 256)}_"
...
async def benchmark(
    ...
    if (
        ...
        result = {
            ...
            "sharegpt_output_len": args_dict.get("sharegpt_output_len"),
            "random_input_len": args_dict.get("random_input_len"),
            "random_output_len": args_dict.get("random_output_len"),
            "random_range_ratio": args_dict.get("random_range_ratio"),
            # Results
...
def run_benchmark(args_dict: dict[str, t.Any]):
    ...
    set_ulimit()
    random.seed(args_dict.get("seed", 1))
    np.random.seed(args_dict.get("seed", 1))

    extra_request_body = {}
    if "extra_request_body" in args_dict and args_dict["extra_request_body"]:
    ...
    if port is None:
        port = {
            ...
            "gserver": 9988,
            "truss": 8080,
        }.get(args_dict.get("backend"), 30000)
    ...
    if backend in ["sglang", "sglang-native"]:
        ...
        api_url = (
            ...
            if base_url
            else f"http://{host}:{port}/v1/completions"
        )
    elif backend == "trt":
        api_url = (
            f"{base_url}/v2/models/ensemble/generate_stream"
            if base_url
            else f"http://{host}:{port}/v2/models/ensemble/generate_stream"
        )
        if "model" not in args_dict or args_dict["model"] is None:
            print("Please provide a model using `--model` when using `trt` backend.")
            sys.exit(1)
    elif backend == "gserver":
        api_url = base_url if base_url else f"{host}:{port}"
        if "model" not in args_dict or args_dict["model"] is None:
            args_dict["model"] = "default"
    elif backend == "truss":
        api_url = (
            f"{base_url}/v1/models/model:predict"
            if base_url
    ...
    # Read dataset
    model_id = model
    tokenizer_id = args_dict.get("tokenizer", model_id)
    tokenizer = get_tokenizer(tokenizer_id)
    input_requests = get_dataset(args_dict, tokenizer)

    flush_cache = args_dict.get("flush_cache", False)
    ...
    return asyncio.run(
        benchmark(
            ...
            tokenizer=tokenizer,
            input_requests=input_requests,
            request_rate=args_dict.get("request_rate", float("inf")),
            max_concurrency=max_concurrency,
            disable_tqdm=args_dict.get("disable_tqdm", False),
            lora_names=args_dict.get("lora_name"),
            extra_request_body=extra_request_body,
            profile=args_dict.get("profile", False),
            pd_separated=args_dict.get("pd_separated", False),
